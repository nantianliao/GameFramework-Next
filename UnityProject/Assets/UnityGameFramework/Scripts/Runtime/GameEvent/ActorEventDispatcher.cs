...
namespace UnityGameFramework.Runtime
{
    ...
    /// 局部单位事件分发器。
    /// </summary>
    public class ActorEventDispatcher : IReference
    {
        /// <summary>
        /// 所有事件。
        /// </summary>
        private readonly Dictionary<int, List<EventRegInfo>> m_AllEventListenerMap;

        /// <summary>
        /// 用于标记一个事件是不是正在处理。
        /// </summary>
        private readonly List<int> m_ProcessEventList;

        /// <summary>
        /// 用于标记一个事件是不是被移除。
        /// </summary>
        private readonly List<int> m_DelayDeleteEventList;

        public ActorEventDispatcher()
        {
            m_ProcessEventList = new List<int>();
            m_DelayDeleteEventList = new List<int>();
            m_AllEventListenerMap = new Dictionary<int, List<EventRegInfo>>();
        }
        ...
        {
            var itr = m_AllEventListenerMap.GetEnumerator();
            while (itr.MoveNext())
            {
                var kv = itr.Current;
            ...
            m_ProcessEventList.Clear();
            m_DelayDeleteEventList.Clear();
            itr.Dispose();
        }
        ...
        {
            if (!m_DelayDeleteEventList.Contains(eventId))
            {
                m_DelayDeleteEventList.Add(eventId);
        ...
        private void CheckDelayDelete(int eventId)
        {
            if (m_DelayDeleteEventList.Contains(eventId))
            {
                ...
                m_DelayDeleteEventList.Remove(eventId);
            }
        }
        ...
        public void SendEvent(int eventId)
        {
            if (m_AllEventListenerMap.TryGetValue(eventId, out var listListener))
            {
                m_ProcessEventList.Add(eventId);
#if UNITY_EDITOR
                int iEventCnt = m_ProcessEventList.Count;
#endif
        ...
        private void AddEventListenerImp(int eventId, Delegate listener, object owner)
        {
            if (!m_AllEventListenerMap.TryGetValue(eventId, out var listListener))
            {
                ...
                m_AllEventListenerMap.Add(eventId, listListener);
            }
    ...
    /// 事件注册信息。
    /// </summary>
    public class EventRegInfo : IReference
    {
        /// <summary>
        /// 事件回调。
        /// </summary>
        ...
        {
            EventRegInfo ret = ReferencePool.Acquire<EventRegInfo>();
